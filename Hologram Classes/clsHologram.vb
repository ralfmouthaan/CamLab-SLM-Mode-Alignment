...
Public MustInherit Class clsHologram
    ...
    Public ReadOnly Property arrHologram As Double(,)
        Get
            ...
            If RawHeight = 0 Then Call Err.Raise(-1, "Hologram Class", "Width has not yet been defined")
                         ...
                         Sub(i)
                             Parallel.For(0, Height,
                                          Sub(j)
                                              ...
                                              'Account for scaling + rotation
                                              RetVal(i, j) = arrRawHologram(CInt(Math.Round((lutnormradius(i, j) * lutcosrotatedtheta(i, j) + 1) * (Width - 1) / 2 / dblScale)),
                                                                            CInt(Math.Round((lutnormradius(i, j) * lutsinrotatedtheta(i, j) + 1) * (Height - 1) / 2 / dblScale)))
    ...
    Public ReadOnly Property arrHologram(ByVal i As Integer, ByVal j As Integer) As Double
        Get
            ...
            If RawWidth = 0 Then Call Err.Raise(-1, "Hologram Class", "Width has not yet been defined")
            If RawHeight = 0 Then Call Err.Raise(-1, "Hologram Class", "Height has not yet been defined")
            ...
            'Account for scaling + rotation
            RetVal = arrRawHologram(CInt(Math.Round((lutnormradius(i, j) * lutcosrotatedtheta(i, j) + 1) * (Width - 1) / 2 / dblScale)),
                                    CInt(Math.Round((lutnormradius(i, j) * lutsinrotatedtheta(i, j) + 1) * (Height - 1) / 2 / dblScale)))
...
#Region "Zernikes"
    Public Property dblScale As Double
        ...
        End Set
    End Property
    Public Property dblRotation As Double
        Get
            Return arrZernikes(9)
        End Get
        Set(value As Double)
            arrZernikes(9) = CDbl(value)
            RedefineLUTs()
        End Set
    End Property
    ...
    Public Sub SaveZernikes(ByVal Filename As String)
        ...
        writer.WriteLine("  Rotation :" & vbTab & dblRotation.ToString)
    ...
    Public Sub LoadZernikes(ByVal Filename As String)
        ...
        dblRotation = CDbl(Split(reader.ReadLine, vbTab)(1))
...
#Region "Height, Width and LUTs"
    ...
    Protected Friend lutsinrotatedtheta(,) As Double
    Protected Friend lutcosrotatedtheta(,) As Double
    ...
    Public Property Height As Integer
        Get
            Return CInt(Math.Round(RawHeight * dblScale)) 'Should I round, ceiling or floor instead?
        End Get
        Set(value As Integer)
            RawHeight = value
        End Set
    ...
    Public Overridable Property RawWidth As Integer
        ...
        End Set
    End Property
    Public Overridable Property RawHeight As Integer
        Get
            Return _RawWidth
        End Get
        Set(value As Integer)
    ...
    Protected Friend Sub RedefineLUTs()
        ...
        ReDim lutsintheta(Width - 1, Height - 1)
        ReDim lutcostheta(Width - 1, Height - 1)
        ReDim lutsinrotatedtheta(Width - 1, Height - 1)
        ReDim lutcosrotatedtheta(Width - 1, Height - 1)
        ReDim lutnormradius(Width - 1, Height - 1)
        ReDim lutradius(Width - 1, Height - 1)

        Dim theta As Double
        Dim rotatedtheta As Double

        For i = 0 To Width - 1
            For j = 0 To Height - 1

                'Calculate r and theta, make hologram circular
                lutnormradius(i, j) = 2 * Math.Sqrt((i - Width / 2) ^ 2 + (j - Height / 2) ^ 2) / Math.Min(Width, Height)
                lutradius(i, j) = 2 * Math.Sqrt((i - Width / 2) ^ 2 + (j - Height / 2) ^ 2) / 1000
                theta = -Math.Atan2(j - Height / 2, i - Width / 2) - Math.PI / 2
                rotatedtheta = theta + dblRotation
                lutsintheta(i, j) = Math.Sin(theta)
                lutcostheta(i, j) = Math.Cos(theta)
                lutsinrotatedtheta(i, j) = Math.Sin(rotatedtheta)
                lutcosrotatedtheta(i, j) = Math.Cos(rotatedtheta)
...
#End Region
    ...
    Public MustOverride Property arrRawHologram As Double(,)
